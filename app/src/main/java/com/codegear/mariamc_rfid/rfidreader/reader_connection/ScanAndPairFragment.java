...
public class ScanAndPairFragment extends DialogFragment implements GoogleApiClient.ConnectionCallbacks, GoogleApiClient.OnConnectionFailedListener {
    ...
    public static ScanAndPairFragment newInstance() {
        ...
        if (scanAndPairFragment == null) return new ScanAndPairFragment();
        else return scanAndPairFragment;
    }
        ...
        scanCode.setOnKeyListener(new View.OnKeyListener() {
            ...
            public boolean onKey(View v, int keyCode, KeyEvent event) {
                if ((event != null && (event.getKeyCode() == KeyEvent.KEYCODE_ENTER))) {
                    ...
                    if (buttonPair != null) buttonPair.performClick();
                }
                return false;
            }
        });
    ...
    void checkForExportPermission() {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
            if (getActivity().checkSelfPermission(Manifest.permission.ACCESS_COARSE_LOCATION) != PackageManager.PERMISSION_GRANTED) {
                requestPermissions(new String[]{Manifest.permission.ACCESS_COARSE_LOCATION, Manifest.permission.ACCESS_FINE_LOCATION, Manifest.permission.ACCESS_BACKGROUND_LOCATION}, 0);
            }
        }
    }

    public void refreshList() {
        if (scanCode != null) scanCode.setEnabled(true);
    ...
    public void removeDevice(String deviceId) {
        ...
        if (mAdapter != null) {
            if (list != null) list.clearChoices();
    ...
    private void showLocationServiceEnablePopup() {
        AlertDialog builder = new AlertDialog.Builder(getContext()).setCancelable(false).setTitle(R.string.Location_Alert_title).setMessage(R.string.warning_text_location_enable).setPositiveButton("Continue", new DialogInterface.OnClickListener() {
            @Override
    ...
    private void stopForegroundDispatch(AppCompatActivity activeDeviceActivity, NfcAdapter mNfcAdapter) {

        if (mNfcAdapter != null) mNfcAdapter.disableForegroundDispatch(activeDeviceActivity);
        //      mNfcAdapter.disableReaderMode(activeDeviceActivity);
    }

    private void setupForegroundDispatch(AppCompatActivity activity, NfcAdapter mNfcAdapter) {
