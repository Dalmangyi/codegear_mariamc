...
public class ScaleActivity extends BaseActivity implements NavigationView.OnNavigationItemSelectedListener, ScannerAppEngine.IScannerAppEngineDevConnectionsDelegate {
    ...
    public void liveWeight() {
        ...
        if (scannerID > 0) {
            ...
            String in_xml = Constants.XMLTAG_ARGXML+ Constants.XMLTAG_SCANNER_ID + getIntent().getIntExtra(Constants.SCANNER_ID, 0) + Constants.XMLTAG_SCANNER_ID+ Constants.XMLTAG_ARGXML;
                        ...
                        switch (event) {
                            ...
                            case XmlPullParser.END_TAG:
                                ...
                                if (outXmlAttribute != null) {
                                    ...
                                    if (outXmlElement.equals(Constants.WEIGHT_XML_ELEMENT)) {
                                    ...
                                    } else if (outXmlElement.equals(Constants.WEIGHT_MODE_XML_ELEMENT)) {
                                    ...
                                    } else if (outXmlElement.equals(Constants.WEIGHT_STATUS_XML_ELEMENT)) {
                                        int status = Integer.parseInt(outXmlAttribute.trim());
                                        String scaleStatus = null;
                                        if (status == STATUS_SCALE_NOT_ENABLED) {
                                            scaleStatus = Constants.SCALE_STATUS_SCALE_NOT_ENABLED;
                                        } else if (status == STATUS_SCALE_NOT_READY) {
                                            scaleStatus = Constants.SCALE_STATUS_SCALE_NOT_READY ;
                                        } else if (status == STATUS_STABLE_WEIGHT_OVER_LIMIT) {
                                            scaleStatus = Constants.SCALE_STATUS_STABLE_WEIGHT_OVER_LIMIT ;
                                        } else if (status == STATUS_STABLE_WEIGHT_UNDER_ZERO) {
                                            scaleStatus = Constants.SCALE_STATUS_STABLE_WEIGHT_UNDER_ZERO;
                                        } else if (status == STATUS_NON_STABLE_WEIGHT) {
                                            scaleStatus = Constants.SCALE_STATUS_NON_STABLE_WEIGHT;
                                        } else if (status == STATUS_STABLE_ZERO_WEIGHT) {
                                            scaleStatus = Constants.SCALE_STATUS_STABLE_ZERO_WEIGHT;
                                        } else if (status == STATUS_STABLE_NON_ZERO_WEIGHT) {
                                            scaleStatus = Constants.SCALE_STATUS_STABLE_NON_ZERO_WEIGHT;
                                        }
