...
//        }
    ...
    public void dcssdkEventBinaryData(byte[] binaryData, int fromScannerID) {
        // todo: implement this
        Constants.logAsMessage(Constants.DEBUG_TYPE.TYPE_DEBUG, TAG, "BinaryData Event received no.of bytes : "+binaryData.length + " for Scanner ID : "+fromScannerID);
    }
    /**
        ...
        public void handleMessage(Message msg) {
            switch(msg.what){
                case Constants.IMAGE_RECEIVED:
                    Constants.logAsMessage(Constants.DEBUG_TYPE.TYPE_DEBUG, TAG, "Image Received");
                    ...
                    for (IScannerAppEngineDevEventsDelegate delegate : mDevEventsDelegates) {
                        if (delegate != null) {
                            Constants.logAsMessage(Constants.DEBUG_TYPE.TYPE_DEBUG, TAG, "Show Image Received");
                            delegate.scannerImageEvent(imageData);
                        }
                    }
                    break;
                case Constants.VIDEO_RECEIVED:
                    Constants.logAsMessage(Constants.DEBUG_TYPE.TYPE_DEBUG, TAG, "Video Received");
                    byte[] videoEvent=(byte[])msg.obj;
                    for (IScannerAppEngineDevEventsDelegate delegate : mDevEventsDelegates) {
                        if (delegate != null) {
                            Constants.logAsMessage(Constants.DEBUG_TYPE.TYPE_DEBUG, TAG, "Show Video Received");
                ...
                case Constants.FW_UPDATE_EVENT:
                    Constants.logAsMessage(Constants.DEBUG_TYPE.TYPE_DEBUG, TAG, "FW_UPDATE_EVENT Received. Client count = "+mDevEventsDelegates.size());
                    FirmwareUpdateEvent firmwareUpdateEvent=(FirmwareUpdateEvent)msg.obj;
                    for (IScannerAppEngineDevEventsDelegate delegate : mDevEventsDelegates) {
                        if (delegate != null) {
                            Constants.logAsMessage(Constants.DEBUG_TYPE.TYPE_DEBUG, TAG, "Show FW_UPDATE_EVENT Received");
                            delegate.scannerFirmwareUpdateEvent(firmwareUpdateEvent);
                        }
                    }
                    break;
                case Constants.BARCODE_RECEIVED:
                    Constants.logAsMessage(Constants.DEBUG_TYPE.TYPE_DEBUG, TAG, "Barcode Received");
                    ...
                    for (IScannerAppEngineDevEventsDelegate delegate : mDevEventsDelegates) {
                        if (delegate != null) {
                            Constants.logAsMessage(Constants.DEBUG_TYPE.TYPE_DEBUG, TAG, "Show Barcode Received");
                ...
                /* update dev list */
                    ...
                    if (!found) {
                        ...
                        Constants.logAsMessage(Constants.DEBUG_TYPE.TYPE_DEBUG, TAG, "ScannerAppEngine:dcssdkEventScannerDisappeared: scanner is not in list");
                    }
